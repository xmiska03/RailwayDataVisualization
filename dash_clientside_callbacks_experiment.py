...
app.layout = html.Div(children=
    [
        dcc.Interval(
            id="animation-interval", interval=250, n_intervals=0, max_intervals=50
        ),
        dcc.Store(
...
def shift_camera(n_intervals):
    new_pos = 10 * n_intervals
    if new_pos >= frames_cnt:
        new_pos = 0    # end of animation, return the the beginning
...
clientside_callback(
    """
    function(new_pos, transf_strings, pcl_data) {
        
        dash_clientside.set_props("point-cloud-store", {data: pcl_data})
    ...
    Output(component_id='camera-position-label', component_property='children'),
    Input(component_id='camera-position-slider', component_property='value'),
    Input(component_id='transf-strings', component_property='data'),
    Input(component_id='point-cloud-store2', component_property='data'),
    prevent_initial_call=True
)
